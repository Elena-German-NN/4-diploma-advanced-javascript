...
export default class GameController {
  ...
  init() {
    this.gamePlay.drawUi(themes.currentTheme);
  ...
  NewGame() {
    GameState.from({score: JSON.stringify(this.score)});
  ...
  levelUp() {
    ...
    if (this.PositionedCharacters.findIndex(item => item.character.level > 2) >= 0) {
      //наипростейшая логика по начислению баллов, нет времени на реализацию полноценного алгоритма      
      if (this.PositionedCharacters.findIndex(item => ['bowman', 'swordsman', 'magician'].includes(item.character.type)) >= 0) {
        ...
        alert(`Вы проиграли! -100 баллов
          Общий счет:
           пользователь ${this.score.player}
           компьютер ${this.score.opponent}`)
      }
      this.NewGame();
    } else {
  ...
  attackCharacter(attacker, attacked) {
    ...
    this.gamePlay.showDamage(attacked.position, damage).then(() => {
      ...
      if (attacked.character.health <= 0) {// если убил      
